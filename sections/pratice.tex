...
\begin{enumerate}
  \item 将每个字符 $m$ 看成一棵只有一个结点的二叉树（叶子结点），其权值为 $f(m)$。
  \item 在所有当前树（或结点）中，\textbf{反复选择权值最小的两棵树} $T_1,T_2$（贪心选择）。
  \item 新建一个父结点，将 $T_1,T_2$ 分别作为其左、右子树，
        父结点权值取为两子树权值之和：$w(\text{parent}) = w(T_1)+w(T_2).$
  \item 用这棵新树替代原来的两棵树，继续步骤 2，直到只剩下一棵树为止，
        该树即为哈夫曼编码树。
  \item 规定从根到叶子的边上左分支记为 $0$、右分支记为 $1$（或反之也可），
...
\noindent\textbf{3.旅行商问题（TSP）：}针对旅行商 TSP 问题，
\begin{enumerate}
