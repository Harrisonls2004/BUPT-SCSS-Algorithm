    ...
    for (int i = n; i >= 1; i--) {        // 区间左端点
        for (int j = i; j <= n; j++) {    // 区间右端点
            if (i == j)
                ...
                int temp = INT_MAX;
                for (int k = i; k < j; k++) {
                    int ans = m[i][k] + m[k+1][j] + p[i-1] * p[k] * p[j];
                    // p[i-1]是k-1的行数；p[k]是列，也是j的行数；p[j]是j的列数
                    if (ans < temp) {
                        temp = ans;
                        s[i][j] = k;      // 记录最优划分点
                    }
...
$$
S[i]=\min_{1\le k\le \min(i,256)}
\Bigl\{S[i-k]+k\cdot b_{\max}(i-k+1,i)\Bigr\}+11.
$$
